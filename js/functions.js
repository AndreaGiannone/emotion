...
function reset_bootstrap_button_style($class_triggered, $result_tr_em = []){

    var $json = '{ "background-color": "#f8f9fa","border-color":"#f8f9fa" }';
    var $cssObject = JSON.parse($json);
    var $selector_and_div = $class_triggered + ' div';
    $($selector_and_div).text($result_tr_em['output']);
    $('button[id=' + $class_triggered + ']').css($cssObject);
...
function unhighlight_column($class_triggered, $result_tr_em = []){

    var $highlight_column = '{ "border-style": "unset","border-color":"unset" }';
    var $cssObject = JSON.parse($highlight_column);
    $('.' + $class_triggered).css($cssObject);

}

function query_font($font_size){

    $('.question div').css('font-size',$font_size);
